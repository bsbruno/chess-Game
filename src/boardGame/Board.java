...
public class Board {
	...
	public Board(int rows, int columns) {
		if (rows < 1 || columns < 1) {
			throw new BoardExecption("Error creating board: there must a be 1 rown and 1 column");
		}
		this.rows = rows;
	...
	public Piece piece(int row, int column) {
		if(!positionExists(row, column)) {
			throw new BoardExecption("Position not on a board");
		}
	...
	public Piece piece(Position position) {
		if(!positionExists( position)) {
			throw new BoardExecption("Position not on a board" + position);
		}
